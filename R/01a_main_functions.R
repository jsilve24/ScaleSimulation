...
aldex_scrv <- function(reads, conds, mc.samples=128, denom=NULL, alpha) {
  d <- nrow(reads)
  n <- ncol(reads)

  ## multinomial dirichlet resampling
  prior <- 0.5
  post.params <- reads+prior
  pi <- array(data=NA, dim=c(d,n,mc.samples))
  for (i in 1:n) {
  ...
  lgm <- apply(lpi, MARGIN=3, colMeans)

  ## sample from scale model
  z <- lwperp <- array(data=NA, dim=c(n,mc.samples))
  z[] <- rnorm(length(z))
  lwperp <- -lgm + alpha*z
  ## rm(z) # garbage collect, included only for readability
  
  ## w = pi * wperp
  lw <- array(data=NA, dim=dim(lpi))
  for (i in 1:d) { # for loop for readability rather than sweep
    lw[i,,] <- lpi[i,,] + lwperp
  }
  rm(lpi, lwperp) # garbage collect

  ## Calculate estimand
  pre.mean <- apply(lw[,conds=="Pre",], MARGIN=3, rowMeans)
...
run_fakeAldex <- function(dat, n_samples=2000, alpha=NULL, denom=NULL, ...){
  ...
  countdata <- t(dat[,-1,drop=F])
  rownames(coldata) <- colnames(countdata) <- paste0("n", 1:ncol(countdata))
  fit <- aldex_scrv(as.matrix(countdata), as.character(dat$Condition), mc.samples=n_samples, alpha=alpha)
  return(fit)
}
